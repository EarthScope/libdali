 ...
 ***************************************************************************/
int
dl_parse_streamlist (DLCP * dlconn, const char * streamlist,
		     const char * defselect)
{
  int count = 0;
  int fields;
  ...
  dl_strparse (streamlist, ",", &ringlist);
  ringptr = ringlist;
  ...
  while (ringptr != 0)
    {
      ...
      fields = dl_strparse (ringptr->element, ":", &reqlist);
      reqptr = reqlist;

      /* Fill in the NET and STA fields */
      if (dl_strparse (reqptr->element, "_", &netstalist) != 2)
	{
	  dl_log_r (dlconn, 2, 0, "not in NET_STA format: %s\n", reqptr->element);
	  count = -1;
	}
      else
	{
	...
	{                   /* Selectors were included */
	  ...
	  if (strlen (reqptr->element) == 0)
	    {
	      dl_log_r (dlconn, 2, 0, "empty selector: %s\n", reqptr->element);
	      count = -1;
	    }
      ...
      else  /* If no specific selectors, use the default */
	{
      ...
      if ( count != -1 )
        {
          dl_addstream(dlconn, net, sta, staselect, -1, 0);
      	  count++;
	}

      /* Free the netstalist (the 'NET_STA' part) */
      dl_strparse (NULL, NULL, &netstalist);
      
      /* Free the reqlist (the 'NET_STA:selector' part) */
      dl_strparse (NULL, NULL, &reqlist);
  ...
  if ( count == 0 )
    {
      dl_log_r (dlconn, 2, 0, "no streams defined in stream list\n");
    }
  else if ( count > 0 )
    {
      dl_log_r (dlconn, 1, 2, "Parsed %d streams from stream list\n", count);
    }

  /* Free the ring list */
  dl_strparse (NULL, NULL, &ringlist);
  
  return count;
}  /* End of dl_parse_streamlist() */
